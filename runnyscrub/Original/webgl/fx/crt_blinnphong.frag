...
void main(void) {
	...
	if (uCrtToggles[2]) {
		...
		//I = Ambient + (L.N)*Diffuse + ((N.H)^Shininess)*Specular
		...
		//Blinn-Phong model calcualts the half vector between L and V. Faster than calculating R
		...
		vec3 I = uAmbientColour + 
				...
				(pow(NdotH, uSpecularShininess) * uPointLightColour);
